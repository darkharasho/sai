// Mobile wire surface — trimmed for Option B (PWA-in-WebView).
// The native shell only needs pairing/unpairing/health + URL helpers.
// All chat/terminal/files traffic runs inside the bundled PWA's own
// WebSocket client, so the heavyweight `connect()` / WireClient and
// per-feature types have been removed.
