...
contextBridge.exposeInMainWorld('sai', {
  ...
  terminalOnCtrlC: (callback: () => void) => {
    const listener = () => callback();
    ipcRenderer.on('terminal:ctrl-c', listener);
    return () => ipcRenderer.removeListener('terminal:ctrl-c', listener);
  },
  ...
  devlog: (event: string, data?: Record<string, unknown>) => ipcRenderer.send('devlog:write', event, data),
  devlogInfo: (): Promise<{ enabled: boolean; path: string | null }> => ipcRenderer.invoke('devlog:info'),
  ...
  homeDir: (): Promise<{ path: string; aliases: string[] }> => ipcRenderer.invoke('app:homeDir'),
