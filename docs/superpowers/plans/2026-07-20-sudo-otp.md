...
- **Mobile remote:** `sudo-prompt` events reach the remote bus but the mobile
  client has no sudo card; a phone-driven session hitting sudo waits out the
  5-minute timeout. Same class as the unmounted-chat limitation.
- **Mount re-seed race:** `claude:sudoPending` snapshots the broker; if the
  prompt resolves between snapshot and render, a stale card could persist
  until remount (submitting is a safe no-op). Narrow window, accepted.
