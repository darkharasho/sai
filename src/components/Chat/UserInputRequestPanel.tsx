...
import type { CodexUserInputAnswers } from '../../../electron/services/codexBackend';

interface Props {
  request: PendingCodexUserInput;
  onSubmit: (answers: CodexUserInputAnswers) => void;
  onCancel: () => void;
}
...
export default function UserInputRequestPanel({ request, onSubmit, onCancel }: Props) {
  ...
  const submittedAnswers = (): CodexUserInputAnswers => Object.fromEntries(request.questions.map(question => {
    const selected = answers[question.id] ?? [];
    const custom = other[question.id]?.trim();
    return [question.id, { answers: custom && (question.allowOther || !question.options) ? [...selected, custom] : selected }];
  }));
  const canSubmit = request.questions.every(question => (submittedAnswers()[question.id]?.answers ?? []).length > 0);
  ...
  return <section className="app-server-input-panel" data-testid="codex-user-input-request">
    ...
    {request.questions.map(question => <fieldset key={question.id}>
      {question.header ? <strong className="app-server-input-header">{question.header}</strong> : null}
      ...
      {question.allowOther && <label>Other<input type={question.isSecret ? 'password' : 'text'} aria-label={`${question.prompt} other`} value={other[question.id] ?? ''} onChange={event => setOther(prev => ({ ...prev, [question.id]: event.target.value.slice(0, 2000) }))} /></label>}
      {!question.options && <label>Answer<input type={question.isSecret ? 'password' : 'text'} aria-label={question.prompt} value={other[question.id] ?? ''} onChange={event => setOther(prev => ({ ...prev, [question.id]: event.target.value.slice(0, 2000) }))} /></label>}
    </fieldset>)}
